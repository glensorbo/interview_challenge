...
export const create = async (entry: IName) => {
  const newEntry = new NameModel(entry);
  return await newEntry.save();
};

export const find = async (query = {}) => {
  return await NameModel.find(query);
};

export const findOne = async (sex: string = 'kombined') => {
  let names = await NameModel.findOne({ sex });
